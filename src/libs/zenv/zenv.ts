// https://github.com/tomanagle/zennv/blob/main/src/index.ts
import { z } from 'zod';
...
export const zenv = <S extends z.AnyZodObject>({
  dotenv = true,
  schema,
  data,
}: Zennv<S>): z.infer<S> => {
  ...
  for (const error of errors.errors) {
    ...
    if (error.code === 'invalid_type') {
      const fnPath =
        `${error.received}_to_${error.expected}` as keyof typeof cast;
